...
contract Strategy is Basic, OneinchCaller {
    ...
    */
    ...
    function enterExitProtocol(bool _isEnter) internal {
        ...
        if (_isEnter){
            executeLendingLogic(ILendingLogic.enterProtocol.selector, '0x');
        } else {
            executeLendingLogic(ILendingLogic.exitProtocol.selector, '0x');
        }
    }
    ...
    function executeDepositWithdraw(address _asset, uint256 _amount, bool _isDeposit) internal {
        if (_amount == 0) return;
        bytes memory callBytes_ = abi.encode(_asset, _amount);
        if (_isDeposit)
